...
/// import android.annotation.*;  /// @Nullable Intent data
/// Term mode Build Error... 240511Sa0248P, -- iian.

import  android.provider.Settings;
import org.apache.http.params.BasicHttpParams;  /// Settings.ACTION_MANAGE_APP_ALL_FILES_ACCESS_PERMISSION
...
public class Basic extends Activity {
    ...
    public static final String JAVA_APP_PATH = "/storage/emulated/0/rfo-basic/Java/App/AppList.dat";
    /// New add...
    ...
    public static String sourcePath = ""; /// ++ iian.
    int bkflag = 0;
    public static String FileName1 = ""; // ++ iian.
    //////////
	...
	public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
		switch (requestCode) {
         ...
         ////  if(Run.checkPermission("INTERNET")){}
                   
		///	 if (grantResults[0] == PackageManager.PERMISSION_GRANTED) /// ???
             if (grantResults[1] == PackageManager.PERMISSION_GRANTED) /// ???
        /// [0] MANAGE_EXTERNAL_STORAGE, WRITE_EXTERNAL_STORAGE [1] ??
        /// [0] READ_EXTERNAL_STORAGE,   WRITE_EXTERNAL_STORAGE [1] ??
        ///  20240512Su1127A, Checked, iian.
                    
         ///      if (Environment.isExternalStorageManager()) /// Crash down...    
               {
			...
			///		finish();  /// Check --- iian.
                ...
                if (isAPK) { createForAPK(); } else { createForSB(); }
                /// Check +++ iian.  soft Start...
                    
				}
                
          }  ///  < A11   /// 240512Zu0854A, ++ iian.
          else   /// >= A11
          {
              /// 240512Zu0855A, ++ iian.
              
              if (!Environment.isExternalStorageManager()) /// O.K.
              {
                  
             Toast.makeText(this, "Allow permission for storage access! (2nd) ", Toast.LENGTH_LONG).show();           
              }
...
///    public static final int MANIFEST_PERMISSION_REQUEST = 0;
    ...
    private void requestPermission() {  /// And 11 ~ 14 ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            try {
                Intent intent = new Intent(Settings.ACTION_MANAGE_APP_ALL_FILES_ACCESS_PERMISSION);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		initVars();
		Settings1.setDefaultValues(this, isAPK);				// if isAPK, force to default Settings1

		defaultTextStyle = new TextStyle();

		String base = Settings1.getBaseDrive(this);
		setFilePaths(base);
	...
	private boolean permissionsGranted() {
        ...
        /*****   20240512Su0636A, --- iian.
                
    if (perm.equals("android.permission.WRITE_EXTERNAL_STORAGE")) 
    {
...
///  protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) 
     protected void onActivityResult(int requestCode, int resultCode, Intent data) 
    {
        ...
        if (requestCode == 2296) 
        {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) 
            {
               ...
               {
                    Toast.makeText(this, "Allow permission for storage access! (1st) ", Toast.LENGTH_LONG).show();
                }
            }
        }
    } /// onActivityResult
   ...
   ////////////////////////////////////////////////
    ...
    boolean AutoStart()  // 220725Mo0334P, iian. ++
    {
        sourcePath = getSourcePath("");

        //       Toast.makeText(this, " • AutoStart • \n" + 
        //                 basePath+ "\n"+
        //                 sourcePath, 
        //     Toast.LENGTH_LONG).show();       

        File file = new File(sourcePath+"/AutoStart.bas");
        if (file.exists())
        {
            return true;
        }
	...
	private void createForSB() {							// Create code for Standard Basic
		...
		Bundle savedState = myIntent.getBundleExtra(Editor_b.EXTRA_RESTART);				// Restart from Editor_b?
		if (FileName == null) {								// Launched with a .bas as argument?
			if (myIntent.getData() != null) FileName = myIntent.getData().getPath();
		}

		if ((FileName != null) && !FileName.equals("")) 
        {				// launched by shortcut or as a file share intent
  ...
  //////////      else 
    ...
    {  // ++ iian.
        ...
        if (AreSamplesLoaded()) 
        {   // this is not a launcher short cut
            
        
        
            boolean atst = AutoStart();  // ++ by iian. 220725Mo0323P.

        FileName1= FileName;  // ++ iian.



        if (atst == true)  // ++ by iian. 220725Mo0420P.
        {

            bkflag =1;

            FileName = sourcePath+  "/AutoStart.bas";
            ...
            delayed (700);  // fixed...
            // 20220725Mo0537P, iian.

        }


        if (bkflag == 1)  // ?? Break Down... Err??  ++ iian.
        {
            bkflag = 0;

            Editor_b.DisplayText = "";   // Clear the display text buffer
            // 20220727We0954P, 1027P. iian.
        }


        DoAutoRun = false;
        Intent intent = new Intent(this, Editor_b.class);

        if (savedState != null) 
        {                                   // if restarted by Editor_b
            intent.putExtra(Editor_b.EXTRA_RESTART, savedState);      // send saved state back
        }


        startActivity(intent);          // to the Editor_b
    ...
    {   // The sample files have not been loaded
	...
	public static void clearProgram() {
		...
		Editor_b.DisplayText = mDefaultFirstLine;				// Display text is the Editor_bs program storage for display
	}
    ...
    {    
       /// JAVA_APP_PATH = 
       /// "/storage/emulated/0/rfo-basic/Java/App/AppList.dat";
    
        File apps = new File (JAVA_APP_PATH);  
        if (apps.exists()) return true;     

        return false;
	}
    
	private static boolean AreSamplesLoaded_Org() {		// Sample program files have not been loaded
	...
	public static String getRelativePath(String targetPath, String basePath) {
      ...
      if (base.length != commonIndex) {
		 ...
		 * If you rotate the device while running the Editor_b,
	...
	public class Loader extends AsyncTask<String, String, String>{
		...
		protected String doInBackground(String... str) {
			...
			startActivity(doNext);			// Start program (APK) or the Editor_b (SB)
...
private Intent doBGforSB() 
    {                               // Background code for Standard Main
    ...
    if (atst == true)  // ++ by iian. 220725Mo0420P.
    {
        ...
        InitDirs();                                         // Initialize Main directories every time
        ...
        if (mRes.getBoolean(R.bool.splash_display)) 
        {       // if displaying splash screen, enforce minimum duration
     ...
     else
    {
           ...
           DoAutoRun = false;
         Intent intent = new Intent(Basic.this, Editor_b.class);
         intent.putExtra(Editor_b.EXTRA_LOADPATH, "../"+SOURCE_DIR);// start in Source_Programs
         // Source dir Start : 20220717Su0142P, ++ iian
        
            return intent;  // go to the Editor_b
     }
  ...
  /////////////////////////////////////////////////
		...
		public void doFirstLoad() {
			...
			Editor_b.DisplayText = openComment +				// Initialize the Display Program Lines
								 initialInfoText +
		...
		public void doCantLoad() {
			...
			// by the Editor_b to indicate the Base Drive is not writable

			Editor_b.DisplayText = getString(R.string.display_text_open_comment) +
	...
	public static class TextStyle {
		...
		public TextStyle() {				// default constructor uses setup.xml values and Preferences Settings1
			refresh();
		}
		...
		public void refresh() {									// set fields from setup.xml values and Preferences Settings1
			Context appContext = mContextMgr.getContext(ContextManager.ACTIVITY_APP);
			getScreenColors(appContext);
			mSize = Settings1.getFont(appContext);
			mTypeface = Settings1.getConsoleTypeface(appContext);
		}

		public boolean getCustomColors(Context appContext, int[] colors) {
			boolean useCustom = Settings1.useCustomColors(appContext);
			if (useCustom) {
				String[] prefs = Settings1.getCustomColors(appContext);
				for (int i = 0; i < 4; ++i) {
		...
		public void getScreenColors(Context appContext) {
			...
			if (getCustomColors(appContext, colors)) {
				...
				String colorSetting = Settings1.getColorScheme(appContext);
	...
	public static class ColoredTextAdapter extends ArrayAdapter<String> {
		...
		public ColoredTextAdapter(Activity activity, ArrayList<String> alist, TextStyle style) {
			super(activity, Settings1.getLOadapter(activity), alist);
			mActivity = activity;
			mList = alist;
			mTextStyle = style;
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (row == null) {
				...
				row = inflater.inflate(Settings1.getLOadapter(mActivity), null);
			}
