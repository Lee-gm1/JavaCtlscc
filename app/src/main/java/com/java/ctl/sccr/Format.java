...
public class Format extends ListActivity {
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		// if BACK key leave original Editor_b text unmodified
		if (keyCode == KeyEvent.KEYCODE_BACK && event.getRepeatCount() == 0) {
			if (theBackground != null) synchronized (Editor_b.DisplayText) {
	...
	public class Background extends AsyncTask<String, String, String> {
		...
		protected void onPreExecute() {
			originalText = Editor_b.DisplayText.split("\n");	// The original text from the Editor_b
		...
		protected void onPostExecute(String result) {				// Done
			synchronized (Editor_b.DisplayText) {						// protect from cancel
				Editor_b.DisplayText = formattedText;					// Transfer the formatted text
				Editor_b.mText.setText(Editor_b.DisplayText);			// to the Editor_b
				Editor_b.mText.setSelection(0, 0);					// Set the cursor at the top
			}
			finish();
		}
